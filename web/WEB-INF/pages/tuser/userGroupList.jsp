...
<body>
									...
									<div class="control-group">
										...
										<div class="controls">
											...
											<button type="submit" class="btn btn-success">
												<i class="icon-ok icon-white"></i> 搜索
											</button>
								...
								<tbody>
									<c:forEach items="${pagedResult.list}" var="user">
										...
										<tr>
											...
											<td><a onclick="editUserGroups(${user.userId},'${user.userName}')" class="btn btn-primary"><i class="icon-plus-sign icon-white"></i> 选择分组</a></td>
										</tr>
									</c:forEach>
								</tbody>
							</table>
	...
	<div id="modal-container-152137" class="modal hide fade modelWidth" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
		<div class="modal-header">
			...
			<h3 id="myModalLabel">为【用户组1】选择分组</h3>
		</div>
		<div class="modal-body" onselectstart="return false;">
				...
				<div class="box">
					...
					<div id="leftbox" class="box-content">
						<c:forEach items="${userGroups}" var="group">
							<a id="${group.id}">${group.groupName}</a>
						</c:forEach>
					</div>
				</div>
			</div>
			<div class="selrole r">
				<div class="box">
					...
					<div id="rightbox" class="box-content"></div>
				</div>
			</div>
		</div>

		<div class="modal-footer">
			<button id="submitGroup" class="btn btn-primary">确定</button>
	...
	window.onload=function (){
        ...
        $('#submitGroup').on('click', function (e) {
        	if(p==0){
	        	...
	        	$('#loadAlert').show();
	        	var str=groups.join(',');
	        	$.post("${contextPath}/user/updateUserGroup",{userId:userId,groupIds:str},function(result){
	        		p=0;
	        		$('#loadAlert').hide();
	        		$("#modal-container-152137").modal("hide");
	        		if(result.state==200){
	        			groupIds[userId]=result.info;
	        			alert("保存成功！");
	        		}else{
	        			alert("保存失败！");
	        		}
	...
	function editUserGroups(uid,uname){
		userId=uid;
		var groups=groupIds[userId];
		$('#leftbox > a').show();
		...
		reRegClick();
		$('#myModalLabel').text('为【'+uname+'】选择分组');
